...
{% set result =set_table_name('klaviyo_flows_tbl_ptrn','%klaviyo%flows','klaviyo_flows_tbl_exclude_ptrn','') %}
...
{% for i in result %}
        ...
        {{extract_nested_value("attributes","archived","boolean")}} as attributes_archived,
        ...
        {{extract_nested_value("attributes","trigger_type","string")}} as attributes_trigger_type,
        ...
        {{unnesting("links")}}
            ...
            where _daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('klaviyo_flows_lookback') }},0) from {{ this }})
            {% endif %}
