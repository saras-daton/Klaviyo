...
{% set result =set_table_name('klaviyo_dropped_email_tbl_ptrn','%klaviyo%dropped_email','klaviyo_dropped_email_tbl_exclude_ptrn','') %}
...
{% for i in result %}
    ...
    {% if is_incremental() %}
            ...
            where _daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('klaviyo_dropped_email_lookback') }},0) from {{ this }})
            {% endif %}
