...
table_pattern=var('klaviyo_received_email_tbl_ptrn'),
exclude=var('klaviyo_received_email_tbl_exclude_ptrn'),
...
{% if var('timezone_conversion_flag') and i.lower() in tables_lowercase_list and i in var('raw_table_timezone_offset_hours') %}
    ...
    {% if is_incremental() %}
            ...
            where _daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('klaviyo_received_email_lookback') }},0) from {{ this }})
            {% endif %}
