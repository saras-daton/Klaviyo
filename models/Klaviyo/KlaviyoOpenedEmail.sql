...
{% set result =set_table_name('klaviyo_opened_email_tbl_ptrn','%klaviyo%opened_email','klaviyo_opened_email_tbl_exclude_ptrn','') %}
...
{% for i in result %}
        ...
        {{extract_nested_value("event_properties","Client_Canonical","string")}} as event_properties_Client_Canonical,
        ...
        {{ unnesting("links") }}
            ...
            where _daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('klaviyo_opened_email_lookback') }},0) from {{ this }})
            {% endif %}
