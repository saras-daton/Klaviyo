...
table_pattern=var('klaviyo_segments_tbl_ptrn'),
exclude=var('klaviyo_segments_tbl_exclude_ptrn'),
...
{% if var('timezone_conversion_flag') and i.lower() in tables_lowercase_list and i in var('raw_table_timezone_offset_hours') %}
    ...
    {% endif %}
        ...
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="attributes.value:updated") }} as {{ dbt.type_timestamp() }}) as _edm_eff_strt_ts,
        {% else %}
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="attributes.updated") }} as {{ dbt.type_timestamp() }}) as _edm_eff_strt_ts,
        {% endif %}
    ...
    {% if is_incremental() %}
            ...
            where _daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('klaviyo_segments_lookback') }},0) from {{ this }})
            {% endif %}
