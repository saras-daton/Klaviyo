{% if var('KlaviyoUnsubscribedFromList') %}
{{ config( enabled = True ) }}
{% else %}
...
{% set result =set_table_name('klaviyo_unsubscribed_from_list_tbl_ptrn','%klaviyo%unsubscribed_from_list','klaviyo_unsubscribed_from_list_tbl_exclude_ptrn','') %}
...
{% for i in result %}
        ...
        _daton_batch_id,
        /*safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="datetime") }} as {{ dbt.type_timestamp() }}) as _edm_eff_strt_ts,*/
        ...
        {{ unnesting("links") }}
            ...
            where _daton_batch_runtime  >= (select coalesce(max(_daton_batch_runtime) - {{ var('klaviyo_unsubscribed_from_list_lookback') }},0) from {{ this }})
            {% endif %}
